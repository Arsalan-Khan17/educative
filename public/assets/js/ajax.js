        ...
        if (request.readyState == 4) {
            ...
            if(response_div == '') {
                if(request.responseText.toLowerCase().search('fail') != -1) {
                    Swal.fire("Failure", JSON.parse(request.responseText).result, "error");
                } else {
                    Swal.fire("Success", JSON.parse(request.responseText).result, "success");
                }
            } else {
            ...
            if(response_div == '') {
                try {
                    ...
                    if(json.status.toLowerCase()=="success") {
                        Swal.fire("Success", json.result, "success");
                    } else {
                        Swal.fire("Failure", json.result, "error");
                    }
                } catch (ex){
                    Swal.fire("Failure", " Error! Unexpected Response", "error");
    ...
    request.onreadystatechange = function() {
        if (request.readyState == 4) {
            ...
            if(request.responseText != ""){
                ...
                Swal.fire("Failure", "No Response..", "error");
                $('#background_fade').fadeOut(function() {
                    $(this).remove();
                });
            }
    ...
    request.onreadystatechange = function() {
        if (request.readyState == 4) {
            ...
            if(request.responseText != ""){
                ...
                Swal.fire("Failure", "No Responce..", "error");
                $('#background_fade').fadeOut(function() {
                    $(this).remove();
                });
            }
